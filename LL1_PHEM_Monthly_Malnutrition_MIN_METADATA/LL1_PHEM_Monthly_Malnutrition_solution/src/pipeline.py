...
dspath = os.path.join(here, '..', '..', 'LL1_PHEM_Monthly_Malnutrition_dataset')
prpath = os.path.join(here, '..', '..', 'LL1_PHEM_Monthly_Malnutrition_problem')
...
LOOK_BACK_WINDOW = 10
TARGET = 'Malnutrition_total Cases'
...
for i,(idx, df) in enumerate(X_train.groupby(level=LEVEL)):
	# if idx != ('Addis Ababa', 'Gulele', 'St. Peter Hospital'):
	# 	continue
	...
	median_value = series[TARGET].median()
	...
	models[idx] = (model, cutoff, median_value)
	# if i >=2:
	# 	break
...
for i,(idx, df) in enumerate(X_test.groupby(level=LEVEL)):
	...
	(M, cutoff, median_value) = models[idx]
	h = int(((df[TIME].max()-cutoff).days)) + GRACE_PERIOD
	# make predictions
	try:
		predictions_df = pd.DataFrame(M.predict(h))
	except Exception as inst:
		if type(inst) == ValueError: 
			if 'from shape' in inst.args[0]:
				# this means that the h (horizon) is longer than lookback
			...
			elif 'Need at least 3 dates to infer frequency' in inst.args[0]:
				# this means there are not enough points to look back
				# fill the predictions with median value from the train observations
				idx = pd.date_range(cutoff, periods=h + GRACE_PERIOD, freq='D')
				predictions_df = pd.DataFrame(data=[median_value]*len(idx), columns=[TARGET], index=idx)
